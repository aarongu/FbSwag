...
window.fbAsyncInit = function() {
			...
			if (dat != null) {
				...
				if ((dat['name']) != null) {
					...
					if (dat.count > 0)
						if (dat.photos != null)
							...
							var nextCall = id + "/albums?offset=" + (offset + 1) + "&limit=1&fields=name,photos.limit(100).since(" + since + ").fields(source,likes.limit(1000),comments.limit(1000),name),count";
				...
				if (photos != null) {
					for (var i = 0; i < photos.length; i++) {
						...
						pictures.push(new Array(dat2.source, likes, title));
						...
						for (var j = 0; j < comments; j++) {
							...
							if (commentor != name) {
								...
								if (!found)
									totalComments.push(new Array(commentor, 1, title));
							}
						}
						index++;
					}
	...
	function getPicturesOld(id, offset, since, pictures, totalLikes, totalComments, name, index, lastIndex) {
		...
		FB.api((id + "/photos/uploaded?limit=2500&since=" + since + "&offset=" + offset + "&fields=likes.limit(1000),comments.limit(1000),source,name"), function(response) {
			for (var i = 0; i < response.data.length; i++) {
				if (index >= limit) {
					processPictureData(id, since, pictures, totalLikes, totalComments, name);
					return;
				...
				if (dat.name != null)
					title = dat.name;
				pictures.push(new Array(dat.source, likes, title));
				for (var j = 0; j < likes; j++) {
	...
	function processPictureData(id, since, pictures, totalLikes, totalComments, name) {
		...
		for (var i = 0; i < index; i++) {
			...
			document.getElementById('uploadedPics').innerHTML += '<div class="container">' + 
																	...
																	'<div class="caption">' + pictures[i][1] + ' likes</div>' + 
																  '</div>';
	...
	function getFinalInfo(id, totalLikes, totalComments, since, pictures, statuses) {
		...
		for (var i = 0; i < index; i++) {
			document.getElementById("overall").innerHTML += '<div class="container"><div class="heading"><h2>' + total[i][0] + 
	...
	for (var i = 0; i < 3; i++) {
		if (picIndex < pictures.length) {
			...
			document.getElementById('uploadedPics').innerHTML+='<div class="container">' +
																	...
																	'<div class="caption">' + pictures[picIndex][1] + ' likes</div>' + 
																'</div>';
			picIndex++;
		} else {
...
function moreFriends() {
	...
	for (var i = 0; i < 3; i++) {
		if (totFIndex < total.length) {
			document.getElementById("overall").innerHTML += '<div class="container"><div class="heading"><h2>' + total[totFIndex][0] + 
...
function changeTab(tab) {
	document.getElementById('center').style.backgroundImage="url('tab_bg.png')";
	...
	document.getElementById('right').style.backgroundImage="url('tab_bg.png')";
	document.getElementById('left').style.backgroundImage="url('tab_bg.png')";
